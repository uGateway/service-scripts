...
if [ $region = "1" ]; then
    ...
    [ -e .region ] && rm .region
    echo "AS923" >> .region
    sudo sed -i '/thethings.network/c\"servers": [ { "server_address": "router.as1.thethingsnetwork", serv_port_up": 1700, "serv_port_down": 1700, "serv_enabled": true } ],' /opt/ttn-gateway/bin/local_conf.json
else
